...
describe('wave one registry seeds', () => {
  it('activates KLMA and daily GigsNews shadow BAU while keeping every source Cowork-owned', () => {
    const sources = waveOneSources(new Date('2026-08-20T10:00:00.000Z'));
    expect(sources.map((item) => item.id)).toEqual(['gigs-news-daily-import','klma-stoke-gig-list','insangel-daily-import']);
    for (const item of sources) { expect(item.shadow).toBe(true); expect(item.writerAuthority).toBe('cowork'); }
    ...
    expect(sources.find((item) => item.id === 'gigs-news-daily-import')).toMatchObject({
      enabled: true, cadence: 'daily', effectiveCadence: 'daily', maxStalenessHours: 26,
      mode: 'append-only', snapshotSemantics: 'incremental', runtimeClass: 'standard',
    });
    expect(sources.find((item) => item.id === 'insangel-daily-import')).toMatchObject({
      enabled: false, sourceRole: 'planned', effectiveCadence: 'manual',
    });
    expect(sources.find((item) => item.id === 'insangel-daily-import')?.nextScanAt).toBeUndefined();
