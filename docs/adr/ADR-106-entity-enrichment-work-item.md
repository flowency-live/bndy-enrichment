...
The external provider seam uses at most two explicit identity-and-location searches followed by one cheap, stateless, schema-constrained reasoning call. Search capture and reasoning are separate so query count, evidence URLs, structured output and cost can be enforced independently. The reasoner receives only the captured evidence allow-list and has no browsing tool. The seam accepts only requested predicates and exact HTTPS citations returned by those searches, records the complete public search/reasoner payload and measured usage, and fails if the reserved item budget is exceeded. It has no credentials, Lambda or schedule until a concrete provider passes the adversarial qualification cohort and sampled live review.
