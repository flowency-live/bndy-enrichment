...
describe('Lemonrock low-cost schedules', () => {
  ...
  it('retains the historical quarantine without an active recovery deployment path', () => {
    const stack = readFileSync(new URL('../lib/bndy-enrichment-stack.ts', import.meta.url), 'utf8');
    ...
    for (const retired of [
      'lemonrock-completion-deploy.yml',
      'lemonrock-eod-completion.yml',
      'lemonrock-quarantine-historical-failures.yml',
      'lemonrock-recovery-deploy.yml',
      'lemonrock-targeted-recovery.yml',
    ]) {
      expect(existsSync(new URL(`../.github/workflows/${retired}`, import.meta.url))).toBe(false);
    }
  });
});
