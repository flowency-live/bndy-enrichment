...
function source(overrides: Partial<GigSource> = {}): GigSource {
  return {
    ...
    health: 'healthy',
    nextScanAt: '2026-08-20T04:00:00.000Z',
    ...overrides,
  };
}
...
class FakeRegistry implements RegistryReader {
  ...
  async get(sourceId: string): Promise<GigSource | null> {
    return this.sources.find((item) => item.id === sourceId) ?? null;
  }

  async queryDue(): Promise<GigSource[]> {
    return this.sources;
  }
...
class FakeQueues implements QueueSender {
  ...
  async send(queueUrl: string, message: SourceScanMessage): Promise<void> {
    this.sent.push({ queueUrl, message });
  }
}

function deps(sources: GigSource[], now = '2026-08-20T05:00:00.000Z') {
  const registry = new FakeRegistry(sources);
  const queues = new FakeQueues();
  const value: DispatcherDependencies = {
    registry,
    queues,
    standardQueueUrl: 'https://sqs.test/standard',
    browserQueueUrl: 'https://sqs.test/browser',
    now: () => new Date(now),
  };
...
describe('source schedule', () => {
  ...
  it('preserves a weekly local weekday from nextScanAt', () => {
    const item = source({
      cadence: 'weekly',
      localTime: '09:00',
      nextScanAt: '2026-08-24T08:00:00.000Z', // Monday 09:00 BST
    });
    expect(nextScheduledAt(item, new Date('2026-08-24T09:00:00.000Z'))).toBe('2026-08-31T08:00:00.000Z');
  });

  it('returns no automatic schedule for manual sources', () => {
    expect(nextScheduledAt(source({ cadence: 'manual' }), new Date())).toBeUndefined();
  });
});
...
describe('SourceDispatcher', () => {
  ...
  it('manual enqueue works for a disabled source and does not alter its schedule', async () => {
    ...
    expect(queues.sent[0]?.queueUrl).toBe('https://sqs.test/browser');
    expect(registry.advances).toHaveLength(0);
  });
});

describe('wave one registry seeds', () => {
  it('seeds all five sources disabled, shadowed and Cowork-owned', () => {
    ...
    expect(sources.map((item) => item.id)).toEqual([
      ...
      'sceniceye-daily-import',
      'insangel-daily-import',
    ]);
    for (const item of sources) {
      expect(item.enabled).toBe(false);
      expect(item.shadow).toBe(true);
      expect(item.writerAuthority).toBe('cowork');
      expect(item.nextScanAt).toBeTruthy();
    }
